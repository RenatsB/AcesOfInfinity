...
void Ship::FollowTarget(float &_deltaTime)
{
    ...
    glm::vec3 toVec = target - position;
    ...
    glm::vec3 tempooo = glm::radians(rotation);
    ...
    rotation = glm::degrees(glm::eulerAngles(temp));
...
void Ship::Evade(float &_deltaTime, Ship *_curTgt, float &_navDist)
{
    ...
    if(target == position) //special case when evasion is called for the first time
    {
        //generate a random point within weapons range from the ship
        do //we have to at least do it once
        {
            target = RandCoords(position); //random coords relative to this position
        }
        while(glm::dot(forward, target-position) > 0); //make sure the evasion point is not right in front of us
        //as this would not make sense and we would hit the objects we are trying to evade
        FollowTarget(_deltaTime);
    }
    else //if we already have a navpoint to follow
    {
        if (_navDist <= actionDistances[0]) //reached evasion coords
        {
            target = _curTgt->position; //set target position back to original ship target
            shipStates = FOLLOW;
        }
...
void Ship::Patrol(float &_deltaTime, Ship *_curTgt, float &_tgtDist, float &_navDist)
{
    ...
    if(_curTgt!=nullptr) //sanity check
    {
        ...
        {
            target = _curTgt->position; //set target as enemy ship position
            shipStates = FOLLOW;
        }
        else //no targets in range but targets present beyond detection range
        {
            //in this case we ARE actually patrolling
            if(_navDist < actionDistances[0]) //reached navpoint position
...
void Ship::Follow(float &_deltaTime, Ship *_curTgt, float &_dist)
{
    ...
    else //enemy still within ranges
    {
        ...
        {
            //switch navpoint to enemy position
            target = _curTgt->position;
...
void Ship::Battle(float &_deltaTime, Ship *_curTgt, float &_dist)
{
    ...
    if (_dist <= actionDistances[1]) //if enemy is still within weapons range
    {
        ...
        {
            if(target != position && shipStates != EVADE) //no evasion activated yet
            {
                fireWeapons = false;
                target = position;
                shipStates = EVADE;
        ...
        {
            target = _curTgt->position;
            if (glm::dot(forward,glm::normalize(target-position)) > 0.8f) //if enemy is in front
            {
                fireWeapons = true;
            }
...
void Ship::FireBullet(glm::vec3 &_curTgt, float &_curTime)
{
    ...
    glm::vec3 toVec = _curTgt - position; //target direction
    ...
    newBullet.position = position + (forward * 0.05f);
    newBullet.rotation = bulRotation;
    newBullet.tag = tag; //the same as this ship's tag
    newBullet.forward = forward; //the same as this ship's forward
    newBullet.dieTime = _curTime+5.0f;
...
void Ship::FlyShip(float &_deltaTime, float &_curTime, float &_score)
{
    ...
    Ship *tmpSh = nullptr;
    objTag tempTags[2] = {tag, tag}; //only necessary for allies
    ...
    if(isPlayer == false) //NPC ship
    {
        ...
        float tgtDistance = glm::distance(position, tmpShEn->position);
        //if any of the closest objects are too close
        if(tgtDistance <= actionDistances[0]||
           glm::distance(position, tmpObj->position) <= actionDistances[0]||
           glm::distance(position, tmpSh->position) <= actionDistances[0])
        {
            if(target != position && shipStates != EVADE) //if no evasion active yet
            {
                target = position;
                shipStates = EVADE;
            }
        ...
        //store this in a variable to avoid multiple duplicate calculations
        float navDistance = glm::distance(position, target);
...
void Ship::CollisionDetect(GameObject *refObj, Ship *refShipEn, Ship *refShipAl, GameObject *refBullet, float &_deltaTime, float &_score)
{
    ...
    if(refObj != nullptr) //for collision with an Asteroid
    {
        ...
        if(glm::distance(refObj->position, position)-refObj->size/2 <= collisionDist)
        {
    ...
    {
        if(glm::distance(refShipEn->position, position) <= collisionDist)
        {
    ...
    {
        if(glm::distance(refShipAl->position, position) <= collisionDist)
        {
            DamageShip(50.0f*_deltaTime, 70.0f*_deltaTime);
        }
    }
    if(refBullet != nullptr) //for collision with a bullet
    {
        ...
        if(glm::distance(refBullet->position, position) <= collisionDist/2)
        {
            DamageShip(15.0f, 10.0f);
            if(refBullet->tag == PLAYER) //if a bullet was shot by player
            {
...
template<typename T1> T1* Ship::GetNearestTgtObj (std::vector<T1>* _allObjs, objTag tagg[], bool _isBul)
{
    ...
    if(!_allObjs->empty()) //sanity check
    {
        ...
        for(int i=0; i<arrSize; ++i)
        {
            ...
            if((_allObjs->at(i).tag == tagg[0] || _allObjs->at(i).tag == tagg[1]) && glm::distance(position, _allObjs->at(i).position) < minDist)
            {
                    ...
                    //when we have many bullets in the scene (can be up to several thousands)
                {
                        ...
                        minDist = glm::distance(position, _allObjs->at(i).position);
                }
